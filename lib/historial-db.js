  ...
  if (!db) {
    ...
    db.exec(`
      CREATE TABLE IF NOT EXISTS anuncios (
        ...
        angulo TEXT,
        modelo TEXT,
        metodo_principal TEXT DEFAULT 'crear',
        briefing_json TEXT,
    ...
    // ── Migración idempotente: columna metodo_principal ──────────
    // better-sqlite3 es síncrono; ALTER falla si la columna ya existe,
    // así que se chequea antes (idempotente al correr varias veces).
    try {
      const colsAnuncios = db.prepare("PRAGMA table_info(anuncios)").all()
      if (!colsAnuncios.some(c => c.name === 'metodo_principal')) {
        db.exec(`ALTER TABLE anuncios ADD COLUMN metodo_principal TEXT DEFAULT 'crear'`)
      }
    } catch (e) { /* columna ya existe */ }

    // Backfill: anuncios con versiones 'metodo1-*' y SIN versiones del
    // modo 'crear' se marcan como 'metodo1'. Idempotente: solo toca los
    // que siguen en 'crear'.
    try {
      db.exec(`
        UPDATE anuncios
        SET metodo_principal = 'metodo1'
        WHERE (metodo_principal IS NULL OR metodo_principal = 'crear')
          AND id IN (SELECT DISTINCT anuncio_id FROM versiones WHERE tipo LIKE 'metodo1-%')
          AND id NOT IN (SELECT DISTINCT anuncio_id FROM versiones WHERE tipo IN ('analisis','generar','correccion','variaciones','auditar'))
      `)
    } catch (e) { /* sin datos que migrar */ }
  }
  return db
}
...
const METODOS_VALIDOS = ['crear', 'metodo1', 'metodo2', 'metodo3']

export function crearAnuncio(briefing = {}) {
  const db = getDB()
  const c = camposBriefing(briefing)
  const ahora = nowIso()
  const metodoPrincipal = METODOS_VALIDOS.includes(briefing.metodoPrincipal)
    ? briefing.metodoPrincipal
    : 'crear'
  const stmt = db.prepare(`
    INSERT INTO anuncios
      (creado_en, actualizado_en, autor, producto, avatar, formato, duracion, tipo_imagen, nivel, motivo, angulo, modelo, metodo_principal, briefing_json, costo_usd_total, costo_cop_total, versiones_count)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0)
  `)
  const res = stmt.run(
    ahora, ahora,
    c.autor, c.producto, c.avatar, c.formato, c.duracion, c.tipo_imagen,
    c.nivel, c.motivo, c.angulo, c.modelo, metodoPrincipal, c.briefing_json
  )
  ...
  if (autor && String(autor).trim()) {
    return db.prepare(`
      SELECT id, creado_en, actualizado_en, autor, producto, avatar, formato, duracion,
             tipo_imagen, nivel, motivo, angulo, modelo, metodo_principal,
