...
function getDB() {
  if (!db) {
    ...
    // Backfill: re-etiqueta anuncios que tienen versiones 'metodo1-*'
    // pero siguen marcados como 'crear' (o NULL). Idempotente: solo
    // toca los que aún no están etiquetados como 'metodo1'.
    try {
      db.exec(`
        UPDATE anuncios
        SET metodo_principal = 'metodo1'
        WHERE id IN (
          SELECT DISTINCT anuncio_id
          FROM versiones
          WHERE tipo LIKE 'metodo1-%'
        )
        AND (metodo_principal IS NULL OR metodo_principal = 'crear')
      `)
